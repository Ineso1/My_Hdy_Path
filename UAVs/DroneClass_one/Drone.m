classdef Drone < DroneDataExtention
    properties
        ...
        %% Translational observer variables 
        A_trans
        B_trans
        C_trans
        B_pinv_trans
        Omega_UDE_trans     % UDE
        xi_UDE_trans        % UDE
        desired_eigenvalues_trans   % L
        L_trans                     % L
        k_d_SMC_trans               % SMC Diff
        Gamma_SMC_trans             % SMC Diff 

        %% Rotational observer variables
        A_rot
        B_rot
        C_rot
        B_pinv_rot
    ...
    methods
        function obj = Drone(mass, Jxx, Jyy, Jzz, q, x0, y0, z0, dt)
            ...
            obj.dx_hat_trans = [0; 0; 0; 0; 0; 0;];
            obj.dx_hat_rot = [0; 0; 0; 0; 0; 0;];
            obj.x_sys_trans = [0; 0; 0; 0; 0; 0;];
            obj.x_sys_rot = [0; 0; 0; 0; 0; 0;];
            ...
            % Initialize observer matrices for translational dynamics 
            ...
            obj.k_d_SMC_trans = diag([15, 15, 15]);
            obj.Gamma_SMC_trans = diag([0.1, 0.1, 0.1]);

            % Initialize observer matrices for rotational dynamics
        ...
        function obj = updateState(obj)
            ...
            [q_vec_0, q_vec_1, q_vec_2, q_vec_3] = parts(obj.dq);
            obj.x_sys_trans = [obj.p; obj.dp;];
            obj.x_sys_rot = [q_vec_1; q_vec_2; q_vec_3; obj.omega];

        end

        function obj = applyControl(obj)
            % Thrust control
            obj = obj.calculateState();
        ...
        function obj = updateDroneDataExtention(obj)
            ...
            obj.disturbance_measure_trans(:, obj.iterations + 1) = obj.w_hat_trans;
            obj.disturbance_measure_rot(:, obj.iterations + 1) = obj.w_hat_rot;
            obj.dx_state_trans_array(:, obj.iterations + 1) = obj.dx_sys_trans;
            obj.dx_state_rot_array(:, obj.iterations + 1) = obj.dx_sys_rot;
            obj.dx_hat_trans_array(:, obj.iterations + 1) = obj.dx_hat_trans;
            obj.dx_hat_rot_array(:, obj.iterations + 1) = obj.dx_hat_rot;
        ...
        function obj = calculateStateUDE_trans(obj)
            ...
            obj.dx_hat_trans = obj.A_trans * x_t + obj.B_trans * obj.u_thrust + obj.B_trans * obj.w_hat_trans;
            obj.x_hat_trans = obj.x_hat_trans + obj.dt * obj.dx_hat_trans;
        end

        function obj = calculateStateUDE_rot(obj)
            %obj.domega = obj.J\(obj.tau - cross(obj.omega, obj.J * obj.omega)) + obj.disturbance_rot;
            controlVar_test = obj.J * obj.u_torque + cross(obj.omega, obj.J * obj.omega);
            [q_vec_0, q_vec_1, q_vec_2, q_vec_3] = parts(obj.q);
            x_r = [q_vec_1; q_vec_2; q_vec_3; obj.omega];
            ...
            obj.x_hat_rot = obj.x_hat_rot + obj.dt * obj.dx_hat_rot;
        end
        ...
        function obj = calculateStateL_trans(obj)
            x_t = [obj.p; obj.dp];
            y = obj.C_trans * x_t;
            obj.dx_hat_trans = obj.A_trans * obj.x_hat_trans + obj.B_trans * (obj.u_thrust - [0; 0; obj.g * obj.mass]) + obj.L_trans * (y - obj.C_trans * obj.x_hat_trans);
            obj.x_hat_trans = obj.x_hat_trans + obj.dt * obj.dx_hat_trans;
            obj.w_hat_trans = x_t - obj.x_hat_trans;
        end
        ...
        function obj = calculateStateL_rot(obj)
            ...
            x_r = [q_vec_1; q_vec_2; q_vec_3; obj.omega];
            y_r = obj.C_rot * x_r;
            obj.dx_hat_rot = obj.A_rot * obj.x_hat_rot + obj.B_rot * obj.tau + obj.L_rot * (y_r - obj.C_rot * obj.x_hat_rot);
            obj.x_hat_rot = obj.x_hat_rot + obj.dt * obj.dx_hat_rot;
            obj.w_hat_rot = x_r - obj.x_hat_rot;
        end

        function obj = calculateStateSMC_trans(obj)
            x_t = [obj.p; obj.dp];
            e_smc_pos = obj.x_hat_trans(1:3) - x_t(1:3);     % Position estimation error
            e_smc_vel = obj.x_hat_trans(4:6) - x_t(4:6);     % Velocity estimation error
            v_injection = -obj.Gamma_SMC_trans * sign(e_smc_pos) - obj.k_d_SMC_trans * e_smc_vel;
            obj.dx_hat_trans(1:3) = obj.x_hat_trans(4:6);     
            obj.dx_hat_trans(4:6) = v_injection;
            obj.x_hat_trans = obj.x_hat_trans + obj.dt * obj.dx_hat_trans;
        end

        function obj = calculateState(obj)
            if obj.observerNumber > 0 && obj.observerNumber < 5
                switch obj.observerNumber
                    ...
                    case 3
                        ...
                        obj = obj.calculateStateSMC_trans();
                    otherwise
                        obj.dx_hat_trans = obj.dx_sys_trans;
                        obj.dx_hat_rot = obj.dx_sys_rot;
                        obj.x_hat_trans = obj.x_sys_trans;
                        obj.x_hat_rot = obj.x_sys_rot;
                end
            else
                obj.dx_hat_trans = obj.dx_sys_trans;
                obj.dx_hat_rot = obj.dx_sys_rot;
