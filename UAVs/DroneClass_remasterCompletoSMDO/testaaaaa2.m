...
L = place(A', C', [-7, -8, -10, -12])';  % Gain for extended observer
...
% Simulation loop
for i = 1:num_steps-1
    % Position error and PD control to compensate estimated disturbance
    error_hat = desired_position - x_hat(1, i);
    error_dot_hat = -x_hat(2, i);
    
    % Control input based on estimated state (compensates estimated disturbance)
    u_hat = Kp * error_hat + Kd * error_dot_hat - 0.8 *x_hat(3, i);  % Compensates for disturbance estimate
